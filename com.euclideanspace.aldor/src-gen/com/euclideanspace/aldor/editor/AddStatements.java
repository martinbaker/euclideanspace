/**
 */
package com.euclideanspace.aldor.editor;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>Add Statements</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * <ul>
 *   <li>{@link com.euclideanspace.aldor.editor.AddStatements#getT1 <em>T1</em>}</li>
 *   <li>{@link com.euclideanspace.aldor.editor.AddStatements#getT13 <em>T13</em>}</li>
 *   <li>{@link com.euclideanspace.aldor.editor.AddStatements#getT14 <em>T14</em>}</li>
 *   <li>{@link com.euclideanspace.aldor.editor.AddStatements#getT15 <em>T15</em>}</li>
 * </ul>
 * </p>
 *
 * @see com.euclideanspace.aldor.editor.EditorPackage#getAddStatements()
 * @model
 * @generated
 */
public interface AddStatements extends EObject
{
  /**
   * Returns the value of the '<em><b>T1</b></em>' containment reference.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>T1</em>' containment reference isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>T1</em>' containment reference.
   * @see #setT1(Expr)
   * @see com.euclideanspace.aldor.editor.EditorPackage#getAddStatements_T1()
   * @model containment="true"
   * @generated
   */
  Expr getT1();

  /**
   * Sets the value of the '{@link com.euclideanspace.aldor.editor.AddStatements#getT1 <em>T1</em>}' containment reference.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>T1</em>' containment reference.
   * @see #getT1()
   * @generated
   */
  void setT1(Expr value);

  /**
   * Returns the value of the '<em><b>T13</b></em>' containment reference.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>T13</em>' containment reference isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>T13</em>' containment reference.
   * @see #setT13(FunctionDefinitionBlock)
   * @see com.euclideanspace.aldor.editor.EditorPackage#getAddStatements_T13()
   * @model containment="true"
   * @generated
   */
  FunctionDefinitionBlock getT13();

  /**
   * Sets the value of the '{@link com.euclideanspace.aldor.editor.AddStatements#getT13 <em>T13</em>}' containment reference.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>T13</em>' containment reference.
   * @see #getT13()
   * @generated
   */
  void setT13(FunctionDefinitionBlock value);

  /**
   * Returns the value of the '<em><b>T14</b></em>' containment reference.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>T14</em>' containment reference isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>T14</em>' containment reference.
   * @see #setT14(FunctionDefinitionBlock)
   * @see com.euclideanspace.aldor.editor.EditorPackage#getAddStatements_T14()
   * @model containment="true"
   * @generated
   */
  FunctionDefinitionBlock getT14();

  /**
   * Sets the value of the '{@link com.euclideanspace.aldor.editor.AddStatements#getT14 <em>T14</em>}' containment reference.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>T14</em>' containment reference.
   * @see #getT14()
   * @generated
   */
  void setT14(FunctionDefinitionBlock value);

  /**
   * Returns the value of the '<em><b>T15</b></em>' containment reference.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>T15</em>' containment reference isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>T15</em>' containment reference.
   * @see #setT15(AddStatements)
   * @see com.euclideanspace.aldor.editor.EditorPackage#getAddStatements_T15()
   * @model containment="true"
   * @generated
   */
  AddStatements getT15();

  /**
   * Sets the value of the '{@link com.euclideanspace.aldor.editor.AddStatements#getT15 <em>T15</em>}' containment reference.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>T15</em>' containment reference.
   * @see #getT15()
   * @generated
   */
  void setT15(AddStatements value);

} // AddStatements
